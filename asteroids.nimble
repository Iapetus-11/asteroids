...
task runr, "Runs asteroids for current platform":
 exec &"nim c -r {releaseOpts} -o:asteroids src/main.nim"

task rund, "Runs debug asteroids for current platform":
 exec &"nim c -r {debugOpts} -o:asteroids src/main.nim"

task release, "Builds asteroids for current platform":
 exec &"nim c {releaseOpts} -o:asteroids src/main.nim"

task webd, "Builds debug asteroids for web":
 exec &"nim c {debugOpts} -d:emscripten -o:asteroids.html src/main.nim"

task webr, "Builds release asteroids for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:asteroids.html src/main.nim"

task debug, "Builds debug asteroids for current platform":
 exec &"nim c {debugOpts} -o:asteroids_debug src/main.nim"
